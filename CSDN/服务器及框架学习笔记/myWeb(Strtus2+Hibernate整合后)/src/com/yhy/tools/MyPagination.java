...
public class MyPagination {
	public List<ShoppingDao> list=null;
	private int recordCount=0;	//记录数
	private int pagesize=0;		//每页显示的记录数
	private int maxPage=0;		//最大页数

	//初始化分页信息
	public List<ShoppingDao> getInitPage(List<ShoppingDao> list,int Page,int pagesize){
		List<ShoppingDao> newList=new ArrayList<ShoppingDao>();
		this.list=list;
		...
		for(int i=(Page-1)*pagesize;i<=Page*pagesize-1;i++){
			try{
				...
				newList.add((ShoppingDao)list.get(i));
		}
		}catch(Exception e){
	...
	//获取指定页的数据
	public List<ShoppingDao> getAppointPage(int Page){
		List<ShoppingDao> newList=new ArrayList<ShoppingDao>();
		try{
			for(int i=(Page-1)*pagesize;i<=Page*pagesize-1;i++){
				try{
