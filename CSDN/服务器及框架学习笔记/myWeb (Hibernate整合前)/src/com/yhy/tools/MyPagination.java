...
public class MyPagination {
	public List<ShoopingDao> list=null;
	private int recordCount=0;	//记录数
	private int pagesize=0;		//每页显示的记录数
	private int maxPage=0;		//最大页数

	//初始化分页信息
	public List<ShoopingDao> getInitPage(List<ShoopingDao> list,int Page,int pagesize){
		List<ShoopingDao> newList=new ArrayList<ShoopingDao>();
		this.list=list;
		...
		for(int i=(Page-1)*pagesize;i<=Page*pagesize-1;i++){
			try{
				...
				newList.add((ShoopingDao)list.get(i));
		}
		}catch(Exception e){
	...
	//获取指定页的数据
	public List<ShoopingDao> getAppointPage(int Page){
		List<ShoopingDao> newList=new ArrayList<ShoopingDao>();
		try{
			for(int i=(Page-1)*pagesize;i<=Page*pagesize-1;i++){
				try{
